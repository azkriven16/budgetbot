...
| 15 | AI Parser Eval Harness | ✅ Done | lib/ai/parse.ts (parseMessage, ParseResult). trigger/parse-message.ts refactored to call callAIParser. vitest installed, vitest.config.ts with @-alias + 30s timeout, tests/ai/setup.ts loads .env.local. tests/ai/parser.test.ts has all 10 golden test cases. test:ai script added to package.json |
...
- **2026-05-23** — Feature 15 (AI Parser Eval Harness): lib/ai/parse.ts extracts generateObject call (parseMessage, ParseResult, internal outputSchema). trigger/parse-message.ts refactored to call callAIParser — schema/model config no longer duplicated. vitest.config.ts with @-alias and 30s timeout. tests/ai/setup.ts loads .env.local without extra packages. tests/ai/parser.test.ts has all 10 golden test cases with live AI calls. pnpm test:ai script added. Review fixed: outputSchema was exported but had no consumer — removed export keyword to keep it module-internal. pnpm build exits 0.
